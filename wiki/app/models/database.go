...
// InitBadgerDB returns a new initialized BadgerDB database implementing the DB
...
func (bdb *BadgerDB) Set(namespace, key, value []byte) error {
	...
	if err != nil {
		revel.AppLog.Debugf("failed to set key %s for namespace %s: %v", key, namespace, err)
		return err
	}
		...
		case <-ticker.C:
			...
			if err != nil {
				...
				if err == badger.ErrNoRewrite {
					revel.AppLog.Debugf("no BadgerDB GC occurred: %v", err)
				} else {
					revel.AppLog.Errorf("failed to GC BadgerDB: %v", err)
				}
			}
